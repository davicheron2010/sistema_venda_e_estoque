...
export function addPurchaseItem(item) {
    purchaseItems.unshift(item); // insere o item novo no topo da lista
    renderPurchaseItems();
}

export function removePurchaseItem(index) {
    if (!Number.isNaN(index) && index >= 0 && index < purchaseItems.length) {
...
export function renderPurchaseItems() {
    ...
    purchaseItems.forEach((item, index) => {
        ...
        removeButton.addEventListener('click', () => removePurchaseItem(index));
